...
/// assert!(str_to_utc_datetime("2023/11/03").is_err());
/// assert!(str_to_utc_datetime("invalid").is_err());
/// ```
...
mod tests {
    ...
    #[test]
    fn test_str_utc_datetime_err() {
        let invalid_date_str = "2023/11/31";
        let invalid_utc_datetime = str_to_utc_datetime(invalid_date_str);
        assert!(invalid_utc_datetime.is_err())
    }
}
