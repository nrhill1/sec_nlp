...
pub fn infer_form_type(input: &str) -> Option<FormType> {
    ...
    for key in [
        ...
        if let Some(val) = extract_after_label(s, key) {
            if let Ok(ft) = FormType::from_str(&val) {
                return Some(ft);
            }
        }
    }
    ...
    if let Some(m) = TOK_RE.captures(s).and_then(|c| c.get(1)) {
        if let Ok(ft) = FormType::from_str(m.as_str()) {
            return Some(ft);
        }
    }
...
PUBLIC DOCUMENT COUNT: 123
        ...
        "#;
        assert_eq!(infer_form_type(input), Some(FormType::TenK));
    }

    #[test]
    fn test_infer_from_json() {
        let input = r#"{"submissionType":"8-K","cik":"0001234567"}"#;
        assert_eq!(infer_form_type(input), Some(FormType::EightK));
    }

    #[test]
    fn test_infer_from_form_keyword() {
        let input = "This is a FORM 10-Q filing for the quarter ending...";
        assert_eq!(infer_form_type(input), Some(FormType::TenQ));
    }

    #[test]
    fn test_infer_sc13d_variants() {
        assert_eq!(infer_form_type("SC 13D"), Some(FormType::Sc13D));
        assert_eq!(infer_form_type("SC13D"), Some(FormType::Sc13D));
    }
    ...
    //     assert_eq!(infer_form_type("FORM 3"), Some(FormType::Form3));
    //     assert_eq!(infer_form_type("FORM 4"), Some(FormType::Form4));
    //     assert_eq!(infer_form_type("FORM 5"), Some(FormType::Form5));
    // }
}
